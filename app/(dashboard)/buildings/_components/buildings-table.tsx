"use client";

import { type ColumnDef } from "@tanstack/react-table";
import { ArrowUpDown, Pencil, Trash } from "lucide-react";
import { Button } from "@/components/ui/button";
import { api } from "@/trpc/react";
import { toast } from "sonner";
import { ConfirmModal } from "@/components/modals/confirm-modal";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@/components/ui/tooltip";

export type BuildingColumn = {
  id: number;
  name: string | null;
  location: string | null;
  created_at: string;
};

interface CellActionProps {
  data: BuildingColumn;
  onEdit: (data: BuildingColumn) => void;
}

const CellAction: React.FC<CellActionProps> = ({
  data,
  onEdit,
}) => {
  const utils = api.useUtils();
  
  const { mutate: deleteBuilding } = api.building.delete.useMutation({
    onSuccess: () => {
      toast.success("Building deleted successfully");
      void utils.building.list.invalidate();
    },
    onError: (error) => {
      toast.error(error.message);
    },
  });

  return (
    <div className="flex items-center gap-2">
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8"
              onClick={() => onEdit(data)}
            >
              <Pencil className="h-4 w-4 text-muted-foreground" />
              <span className="sr-only">Edit {data.name}</span>
            </Button>
          </TooltipTrigger>
          <TooltipContent>
            <p>Edit building</p>
          </TooltipContent>
        </Tooltip>

        <Tooltip>
          <TooltipTrigger asChild>
            <ConfirmModal 
              onConfirm={() => deleteBuilding(data.id)}
              itemToConfirm={data.name ?? ""}
              title="Delete Building"
              description="This action cannot be undone. To confirm deletion, please enter the building name"
              confirmText="Delete Building"
            >
              <Button
                variant="ghost"
                size="icon"
                className="h-8 w-8"
              >
                <Trash className="h-4 w-4 text-destructive" />
                <span className="sr-only">Delete {data.name}</span>
              </Button>
            </ConfirmModal>
          </TooltipTrigger>
          <TooltipContent>
            <p>Delete building</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    </div>
  );
};

export const columns = (onEdit: (data: BuildingColumn) => void): ColumnDef<BuildingColumn>[] => [
  {
    accessorKey: "name",
    header: ({ column }) => {
      return (
        <Button
          variant="ghost"
          onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
        >
          Name
          <ArrowUpDown className="ml-2 h-4 w-4" />
        </Button>
      );
    },
  },
  {
    accessorKey: "location",
    header: ({ column }) => {
      return (
        <Button
          variant="ghost"
          onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
        >
          Location
          <ArrowUpDown className="ml-2 h-4 w-4" />
        </Button>
      );
    },
  },
  {
    accessorKey: "created_at",
    header: "Date Created",
    cell: ({ row }) => {
      const date = new Date(row.getValue("created_at") as string);
      return <div>{date.toLocaleDateString()}</div>;
    },
  },
  {
    id: "actions",
    cell: ({ row }) => <CellAction data={row.original as BuildingColumn} onEdit={onEdit} />,
  },
];
