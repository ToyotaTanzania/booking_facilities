...
export function BookingsCalendar() {
  ...
  const { data: bookings, isLoading: bookingsLoading } = api.booking.getCalendarBookings.useQuery()
  ...
  const events: IEvent[] = bookings?.map((booking: BookingData) => {
    ...
    return {
      id: startDate.getTime() + endDate.getTime() + booking.slot.id + booking.schedule + booking.facility.id,
      startDate: startDate.toISOString(),
      endDate: endDate.toISOString(),
      title: `${booking.facility.name} - ${booking.user.email}`,
      color: booking.status==="confirmed" ? "green" : (booking.status ==="rejected") ? "red" : "yellow",
  ...
  return (
    <CalendarProvider users={users?.map((user: UserData) => ({
      id: user.userid,
      name: user.name,
