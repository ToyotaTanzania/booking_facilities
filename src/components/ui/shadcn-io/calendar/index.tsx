      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex flex-col">
          <span className="truncate font-medium">
            {feature.facilityName ?? feature.name}
          </span>
          <span className="text-xs text-muted-foreground">
            {[feature.buildingName, feature.locationName].filter(Boolean).join(' • ')}
          </span>
          <span className="text-xs">
            {format(new Date(feature.startAt), 'HH:mm')} – {format(new Date(feature.endAt), 'HH:mm')}
          </span>
          {feature.requester && (
            <span className="text-xs">By {feature.requester}</span>
          )}
        </div>
      </div>
      <span
        className="text-xs px-2 py-0.5 rounded-full border"
        style={{ borderColor: feature.status.color, color: feature.status.color }}
      >
        {feature.status.name}
      </span>
    </div>
